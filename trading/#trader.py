...
import math
from numba import njit  # Added for Kalman/Hurst speed
...
HURST_THRESHOLD = 0.6  # Filter trending spreads (>0.5 is trending, <0.5 is mean reverting)

TELEGRAM_BOT_TOKEN = "8575237777:AAHDxDYRu-m_bpb9vb2lfCpIDETXpBEdUtU"  #HARDCODED BUT REMOVE BEFORE PUTTING ON GITHUB
...
# global Settings
trade_interval = 60  #
sleepSeconds = 300  # sleep for 5 minutes when market is closed
tele_interval = 180  # telegram heartbeat interval
assigned_cptl = 10000  # assigned capital per trade.
...
def send_tele(message, alert_type="INFO"):
    ...
    full_message = f"{emoji_map.get(alert_type, '💬')} *[{alert_type}]*\n{message}"
...
def filters_data(asset_a, asset_b, lookback, timeframe=TimeFrame(15, TimeFrameUnit.Minute)):
    try:
        ...
        # need extra data for kalman warmup
...
def calculateSignal(data):
    # run kalman filter for dynamic beta
    Y = data['Log_A'].values
    X = data['Log_B'].values
    beta_series = calculate_kalman_beta(Y, X)

    latest_beta = beta_series[-1]

    # calculate spread based on dynamic beta
    # we stick to simple OLS-style spread centering for Z-score
    # but use the Kalman Beta for the slope

    # Recalculate spread series for Z-score
    data['Kalman_Beta'] = beta_series
    data['Spread'] = data['Log_A'] - data['Kalman_Beta'] * data['Log_B']

    # Hurst on the Spread
    spread_values = data['Spread'].values
    hurst_val = calculate_rolling_hurst(spread_values, window=100)[-1]

    # Rolling Z-Score
    # we use a shorter window for Z-score to be reactive
    z_window = 100 
...
def get_price(asset_a, asset_b):
    ...
    for sym in symbols:
        if not np.isnan(price_map[sym]):
            continue

        try:
            print(f"Falling back to yfinance for {sym}...")
            df = yf.download(sym, period="2d", interval="1m", progress=False, prepost=True, auto_adjust=False)

            if df is None or df.empty:
                print(f"yfinance returned empty data for {sym}")
                continue

            close_series = df["Close"].dropna()

            if isinstance(close_series.iloc[-1], pd.Series):
                last_price = float(close_series.iloc[-1].iloc[-1])
            else:
                last_price = float(close_series.iloc[-1])

            price_map[sym] = last_price
            print(f"Price {sym} (yfinance): ${last_price:.2f}")

        except Exception as e:
            print(f"yfinance fallback failed for {sym}: {type(e).__name__}: {e}")
...
def submit_order(symbol, qty, side, limit_price=None, wait_interval=1, max_wait_seconds=20):
    ...
    If no limit_price is provided, we auto-select:
        - BUY  → use current ask
        - SELL → use current bid
    """
...
def liveLoop():
    ...
    print(
        f"Running Pairs Trader. Check interval: {trade_interval}s. ADF Filter Threshold: P-Value <= {ADF_THRESHOLD}. Hurst < {HURST_THRESHOLD}")
    ...
    while True:
        ...
        try:
            ...
            # regime filter for ADF (TRADER 1 LOGIC: 3-Strike Rule)
            if np.isnan(p_value) or p_value > ADF_THRESHOLD:

                bad_regime_counter += 1
                print(f"⚠️ SUSPICIOUS ADF DETECTED: P-Value {p_value:.4f}. Strike {bad_regime_counter}/3")

                if bad_regime_counter >= 3:
                    # non-stationary regime detected PERSISTENTLY
                    if position != 0:
                        print(
                            f"🟡 REGIME SHIFT CONFIRMED: P-Value {p_value:.4f} > {ADF_THRESHOLD} for 3 checks. Liquidating.")
                ...
                # entry logic
                    ...
                    # NEW FILTER: Hurst Check
                    if abs(current_z) >= Z_ENTRY and Z_STOP_LOSS >= abs(current_z) and hurst < HURST_THRESHOLD:
...
def debug_model():
    ...
    beta2, z2, std2, latest_spread2, pv2, hurst2 = calculateSignal(data)  # Updated unpack
    ...
    for i in range(50, 150):
        temp = tail.iloc[:i]
        _, ztemp, _, _, _, _ = calculateSignal(temp)  # Updated unpack
        zs.append(ztemp)
