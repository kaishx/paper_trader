...
def send_tele(message, alert_type="INFO"):
    ...
    try:
        ...
        print(f"❌ TELEGRAM API ERROR ({http_err.response.status_code}): Failed to send message.")
...
def load_optimal_params(file_path='optimized_params.json'):
    ...
    try:
        ...
        if not found:
            print(f"⚠️ WARNING: No optimized parameters found for {ASSET_A}/{ASSET_B} in '{file_path}'.")
        ...
        print(f"❌ ERROR: Parameter file '{file_path}' not found.")
        return False
    except Exception as e:
        print(f"❌ ERROR: Failed to load parameters from JSON: {e}")
        return False
...
def get_raw(symbol, start, end, timeframe):
    ...
    for attempt in range(3):
        try:
            ...
            print(f"⚠️ Alpaca attempt {attempt+1} failed for {symbol}: {e}")
...
def get_latest_prices_realtime(asset_a, asset_b):
    ...
    print("⏳ Fetching latest price context via REST API...")
    ...
    try:
        ...
        for symbol in symbols:
            ...
            if quote and quote.bid_price > 0 and quote.ask_price > 0:
                ...
                print(f"✅ Price {symbol} (Mid-Quote): ${mid_price:.2f}")
                ...
                print(f"⚠️ Could not get latest quote data for {symbol}.")

        # return the prices
        return price_a, price_b

    except Exception as e:
        print(f"❌ Error fetching latest price context via REST: {e}")
        return price_a, price_b
...
def submit_order(symbol, qty, side, wait_interval=1, max_wait_seconds=15):
    ...
    try:
        ...
        print(f"✅ ORDER PLACED: {side.value} {qty:.4f} shares of {symbol}")
        ...
        while total_waited < max_wait_seconds:
            ...
            # log partial fills
            if 0 < filled_qty < qty:
                print(f"⚠️ PARTIAL FILL: {filled_qty:.4f}/{qty:.4f} shares of {symbol} executed so far.")
        ...
        print(f"✅ FINAL EXECUTION: {filled_qty:.4f}/{qty:.4f} shares of {symbol} executed.")
...
def log_status(current_z, beta, p_value, price_a, price_b, position, pos_qty_a, pos_qty_b, portfolio_equity,
    ...
    status_msg = (
        ...
        f"👉 Position: {position} ({ASSET_A}: {pos_qty_a:.0f} shares, {ASSET_B}: {pos_qty_b:.0f} shares) | Equity: ${portfolio_equity:,.2f}"
    )
    print(status_msg)
...
def liveLoop():
    ...
    print(
        f"🚀 Running Pairs Trader. Check interval: {MIN_TRADE_INTERVAL_SECONDS}s. ADF Filter Threshold: P-Value <= {ADF_THRESHOLD}")
    ...
    while True:
        ...
        try:
            ...
            print(f"👉 Pairs Position Status: {position} ({ASSET_A}: {pos_qty_a:.0f}, {ASSET_B}: {pos_qty_b:.0f})")
            print("-----------------------------------")


        except Exception as e:
            print(f"⚠️ Warning: Could not fetch account statistics. Error: {e}")
        ...
        if not is_market_open_rth:
            print(f"\n😴 MARKET CLOSED: {market_time_str}. Next open: {next_open_str}. Current position: {position}")
        ...
        try:
            ...
            if np.isnan(p_value) or p_value > ADF_THRESHOLD:
                ...
                if position != 0:
                    print(f"🟡 REGIME SHIFT: P-Value {p_value:.4f} > {ADF_THRESHOLD}. Liquidating pair.")
                ...
                if position != 0:
                    ...
                    if is_mean_reversion_exit or is_stop_loss_exit:
                        if is_mean_reversion_exit:
                            print(f"📉 EXIT SIGNAL: Z-Score {current_z:.2f} is within |Z_EXIT|={Z_EXIT}. Closing position.")
                            liquidate(reason=f"Mean-reversion exit: Z-Score {current_z:.2f} within |Z_EXIT|={Z_EXIT}")
                        elif is_stop_loss_exit:
                            print(f"🛑 STOP LOSS: Z-Score {current_z:.2f} is beyond |Z_STOP_LOSS|={Z_STOP_LOSS}. Liquidating NOW.")
