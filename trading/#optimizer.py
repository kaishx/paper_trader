...
API_KEY_ID = "PKVJ2W4CMUA7QLGTLK5V4HTYDQ"  # HARDCODED BUT REMOVE BEFORE PUTTING ON GITHUB
API_SECRET_KEY = "p5WGXeEJs5LYmW6pDGVGQUGevoK9DMcUGsvYgyBHSys"  # HARDCODED BUT REMOVE BEFORE PUTTING ON GITHUB
...
lookback_days = 250
# Reduced rolling window for Z-score centering to match Kalman responsiveness
rolling_window = 100
HURST_THRESHOLD = 0.6
...
# --- KALMAN FILTER (MATCHING TRADER.PY) ---
@njit
...
# --- HURST EXPONENT (MATCHING TRADER.PY) ---
@njit
...
def main_calc(data: pd.DataFrame, window: int) -> pd.DataFrame:
    """
    Calculates Kalman Beta, Spread, Hurst, and Z-Score.
    Using 'window' for Z-score rolling statistics.
    """
    Y = data['Log_A'].values
    X = data['Log_B'].values
    ...
    # 2. Dynamic Spread
    # Note: We align with trader.py logic: Spread = Log_A - Beta * Log_B
    data['Spread'] = data['Log_A'] - data['Beta'] * data['Log_B']

    # 3. Rolling Hurst
    # Fixed window of 100 used in trader, can match that or use 'window' if desired.
    # Sticking to 100 as per trader.py defaults.
...
@njit(cache=True)
def numba_strat(z_scores, betas, close_a, close_b, hurst, z_entry, z_exit, z_stop_loss, hurst_threshold,
    ...
    for i in range(1, len(z_scores)):
        ...
        # --- ENTRY LOGIC ---
        if not in_trade:
            # HURST FILTER CHECK ADDED HERE
            if abs(z) >= z_entry and h < hurst_threshold:

                capital_per_leg = current_equity / 2.0

                # Long Spread (Z is low, buy A sell B? No, Spread = A - Beta*B)
                # If Z is negative (Low Spread), A is cheap vs B. Buy A, Sell B.
                # Wait, typically "Long Spread" means Buy Spread.
                # Spread = A - B. If Spread is -3, it needs to go up. We Buy Spread.
                # Buy A, Sell B.

                if z <= -z_entry:
                    # Long Spread logic
                    qty_a = int(capital_per_leg / price_a)  # Long A
                    qty_b = int((capital_per_leg / price_b) / beta)  # Short B
                    ...
                    # Short Spread logic
                    qty_a = int(capital_per_leg / price_a)  # Short A
                    qty_b = int((capital_per_leg / price_b) / beta)  # Long B
...
def backtest(data: pd.DataFrame, z_entry, z_exit, z_stop_loss):
    ...
    equity_curve = numba_strat(
        ...
        z_entry, z_exit, z_stop_loss, HURST_THRESHOLD,
        CAPITAL, tx_fee, slippage
    )
...
def run_opt(data: pd.DataFrame, asset_a: str, asset_b: str) -> Dict[str, Any]:
    # Updated ranges for Kalman (typically requires lower thresholds than OLS)
